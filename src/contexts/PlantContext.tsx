...
export const PlantContextProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    const projectInStorage = window.localStorage.getItem(StorageKey.PROJECT);
  ...
  useEffect(() => {
    ...
    try {
      matchedProject = matchProjectInURL(availableProjects, params.project);
    ...
    (async (): Promise<void> => {
      ...
      try {
        ...
        if (projectsFromApi.length < 1) {
          setFetchProjectsAndPermissionsStatus(AsyncStatus.EMPTY_RESPONSE);
        } else {
